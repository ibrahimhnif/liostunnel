...
> Naming note: "LiosTunnel" (Indonesian for "corridor/alley") is a placeholder project name used throughout this doc. Swap it out with `find/replace` once you've picked a real name.
...
- **Cannot** be done from the main Flutter app process. Requires:
  ...
  - Requires the `com.apple.developer.networking.networkextension` entitlement — must be requested from Apple (packet-tunnel-provider entry), approval is not automatic for new developer accounts.
...
**Phase 3 — iOS**
Network Extension target, entitlement request from Apple (start this early — approval lead time is unpredictable), App Group state sharing.
...
- **Apple entitlement approval timeline** for `networkextension` is the single biggest schedule risk — apply as early as Phase 1, not Phase 3.
